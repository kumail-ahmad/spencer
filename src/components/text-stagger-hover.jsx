...
export function useAnimationVariants(animation) {
...
const TextStaggerHoverContext = React.createContext(undefined);
...
export const TextStaggerHoverHidden = ({
  ...
  return (
    <span
      className={cn('inline-block absolute left-0 top-0 text-nowrap', className)}>
      {characters.map((char, index) => {
        const staggerDelay = setStaggerDirection({
          direction: staggerDirection,
          totalItems: characterCount,
